...
難し……。解説見てもなかなか分からなかった。部分木DP。

ラベルを以下のように定義する。
$$
\begin{align*}
\alpha(i) &= (iの連結成分と部分木の共通部分にはaのみが含まれる),\\
\beta(i) &= (iの連結成分と部分木の共通部分にはbのみが含まれる),\\
\gamma(i) &= (iの連結成分と部分木の共通部分にはa,bが両方含まれる)\\
\end{align*}
$$

$$
\begin{align*}
dp[i][\alpha] &= (iを根とする部分木の全域部分グラフで、iの連結成分はaのみ、他の全ての連結成分はaとbの両方を含むものの個数)
dp[i][\beta] &=  (iを根とする部分木の全域部分グラフで、iの連結成分はbのみ、他の全ての連結成分はaとbの両方を含むものの個数)
dp[i][\gamma] &= (iを根とする部分木の全域部分グラフで、全ての連結成分がaとbの両方を含むものの個数)
\end{align*}
$$

明らかに $i$ が `a` なら $dp[i][\beta] = 0$。

子→親（`a`の場合）への寄与

- $\alpha$ → $\alpha$: 「繋がっている」だけ。もし繋がっていなければ子に `a` のみの連結成分ができてしまう。
- $\beta$ → $\alpha$: 0。繋がっていれば親の連結成分に `b` が入ってしまう。繋がっていなければ `b` のみの連結成分ができてしまう。
- $\gamma$ → $\alpha$: 「繋がっていない」だけ。もし繋がっていれば親の連結成分に `b` が入ってしまう。

親が `a` の場合に $\beta$ になることはない。

- $\alpha$ → $\gamma$: 「繋がっている」だけ。もし繋がっていなければ子に `a` のみの連結成分ができてしまう。
- $\beta$ → $\gamma$: 「繋がっている」だけ。もし繋がっていなければ子に `b` のみの連結成分ができてしまう。
- $\gamma$ → $\gamma$: 「繋がっている」「繋がっていない」両方可。

ただし、$\beta$ からの寄与がなく、子の $\gamma$ が繋がっていない場合、親の連結成分が `a` だけになってしまうのでこれを除く。

$$
\begin{align*}
dp[i][\alpha] &= \prod_{j: iの子} dp[j][\alpha] + dp[j][\gamma],\\
dp[i][\beta] &= 0,\\
dp[i][\gamma] &= \prod_{j: iの子} \left( dp[j][\alpha] + dp[j][\beta] + 2dp[j][\gamma]  \right)
 - \prod_{j: iの子} \left( dp[j][\alpha] + dp[j][\gamma] \right),\\
\end{align*}
$$

`b` の場合も同様。頂点0を全体の根として、$dp[0][\gamma]$ が答え。
